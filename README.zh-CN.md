...
*1 分钟内过完一生：选蛋 → 喂食/摸摸/洗澡 → 和小火龙打满一场三局两胜猜拳（晃拳、亮牌、终局）→ 生涯数据面板 → 最后，一座墓碑。全部录制自真实运行输出 —— 本 README 里没有任何示意图/手绘 mock。*
...
| 生涯数据面板 | 告别（这一只的） | 按键速查 |
|:---:|:---:|:---:|
| ![统计](docs/assets/stats.png) | ![墓园](docs/assets/grave.png) | ![按键说明](docs/assets/help.png) |
...
- **一个文件，什么都没有。** `pet.mjs` 就是整个程序。无安装脚本、无 package.json、无网络、无遥测。一口气就能读完（约 1200 行）。
...
**一起玩的事**
- **猜拳** —— `g` 开三局两胜（`1`/`2`/`3` 出招）；每次出招都有晃拳→亮牌的像素揭示动画 —— 胜方的手高亮抬起、败方压暗。每个物种有可学习的出招偏好，暮年更固执，而且无论输赢宠物都很开心
...
| `1` | 零食 | 饱腹 +8，心情 +12，更易发胖 —— 饱腹 85 以上才拒绝；1 小时内第 3 次会闹肚子（只能吃药治） |
| `w` | 喂水 | 口渴 +35 |
| `b` | 洗澡 | 洁净 +45，治本（痊愈），饱腹 −6、口渴 −6 —— 热水澡，越洗越饿越渴 |
| `d` | 吃药 | 生病时药到病除（闹肚子的唯一解），心情 −20、精力 −15 —— 脏病不洗澡 1 小时后复发 |
| `p` | 玩耍 | 心情 +28，精力 −15，洁净 −5（玩得一身泥），燃脂 −0.2kg —— 饱腹/口渴 <20 或弥留时拒绝 |
| `s` | 睡觉（切换） | 精力快速恢复（睡 40/h；醒 4/h），睡着饿得慢 —— 睡着时其他动作全拒 |
| `t` | 摸摸 | 心情 +10（冷却 60 秒）—— 也可以直接点它；滚动 1 小时内前 2 次有效，超出心情 −3（暮年 3 次，每次 +12） |
| `g` | 猜拳 | 三局两胜，`1`/`2`/`3` 出招、`q` 罢手 —— 赢：心情 +15、精力 −8、照顾 +1 · 输：心情 +5 · 冷却 3 分钟 |
| `c` | 闲聊 | 心情 +2（冷却 60 秒）；话题随状态、年龄、成就变化 |
| `Tab` | 数据面板 | 生涯统计、成就进度、纪念墙、完整日志 |
| `?` | 按键说明 | 全部按键速查（`?` 或 `Tab` 关闭）；孵化后首次会气泡提示一次 |
...
| 生病 | 三来源：脏病（洁净 <25 持续 1 小时）· 着凉（洁净 <30 每小时 20% 掷骰）· 闹肚子（1 小时内第 3 次零食）→ 食欲减半、心情封顶；洗澡治脏病/着凉，闹肚子只能吃药 |
| 弥留 | 饱腹且口渴双 0 → 4 小时救援窗口（喂食+喂水）；全面崩溃（精力心情也 0）压缩至 1 小时 |
| 死亡计时 | 各绝症状态（心情 0 · 生病 · 1.6× 超重 · 醒着精力 0）2 小时未处理即死 —— 屏幕上有红色倒计时角标；超重可玩耍燃脂退线自救（见运动热度） |
| 运动热度 | 10 分钟内连续玩耍叠 1 层（至多 3 层），体重衰减额外 +0.1×层/h，停玩 20 分钟后恢复 —— 🔥 角标显示层数 |
...
- **pane 太窄？** 布局自动适配：窄于 68 列按钮条切单字速记（全称见 `?` 帮助），小于 58 列 × 25 行会整屏提示、拉大自动恢复；`PET_WIDTH=60 pet` 强制宽度。
- **会联网吗？** 永远不会。无网络、无遥测。
- **英文界面？** 暂无 —— 游戏内文案目前是中文；英文界面在路线图上（[#1](https://github.com/ntexplorer/cli-pet/issues/1)），欢迎搭把手。
